    ...
    <Row>
      <Card>
        ...
        <div style="font-size:12px;">
          <p>
            注意：此页面新建活动仅仅是为了将所有活动聚合以供展示，实际活动的业务与此处无关
          </p>
          <p>活动发起先行条件：</p>
          <p>
            【抢购】与【分摊】需要在【币币设置】中新建交易对时对活动进行设置
          </p>
          <p>【持仓瓜分】与【认购】需要在【活动管理】中新建新的活动</p>
          <br />
          <p>
            1、抢购：所有用户【抢购】指定数量的币，首次上线交易对可指定该类别，需要在【币币设置】中添加新交易对时确定活动内容
          </p>
          <p>
            2、分摊：所有用户【平分】指定数量的币，首次上线交易对可指定该类别，需要在【币币设置】中添加新交易对时确定活动内容
          </p>
          <p>
            3、持仓瓜分：任意时段可发起的活动，活动前，快照用户持仓量，根据用户持仓量瓜分指定数量的币
          </p>
          <p>
            4、认购：一般做首发类型的活动，属于IEO认购类型，所有用户抢购指定数量的币
          </p>
        </div>
      </Card>
    </Row>
...
import defaultImg from "@/images/jys.png";
export default {
  data() {
    return {
      ...
      columns: [
        ...
        },
        {
          ...
          render: (h, obj) => {
            let sTime =
              obj.row.startTime.substr(0, 10) +
              " " +
              obj.row.startTime.substr(11, 8);
            let curTime = new Date().Format("yyyy-MM-dd HH:mm:ss");
            let text = "即将开始";
            if (sTime < curTime) {
              text = "已开始";
            }
            return h("span", {}, sTime + text);
          },
        },
        {
          title: "结束时间",
          ...
          render: (h, obj) => {
            let eTime =
              obj.row.endTime.substr(0, 10) +
              " " +
              obj.row.endTime.substr(11, 8);
            let curTime = new Date().Format("yyyy-MM-dd HH:mm:ss");
            let color = "#19be6b";
            let text = "未结束";
            if (eTime < curTime) {
  ...
  methods: {
    getDetailOrderList() {
      activityOrderList(this.id).then((res) => {
    ...
    async updateList(filterData) {
      const updatedList = await Promise.all(
        filterData.map(async (item) => {
          if(item.smallImageUrl !== ''){
            util.imageIsExist(item.smallImageUrl).then(r=>{
            }).catch(err=>{
              item.smallImageUrl = defaultImg;
            })
          }else{
            item.smallImageUrl = defaultImg;
          }
    ...
    refreshPage(obj) {
      ...
      activityList(this.filterSearch).then((res) => {
        if (!res.code) {
          ...
          let filterData = (res.data && res.data.content) || [];
          // this.list = filterData
          this.updateList(filterData);
        } else {
          this.$Message.error(res.message);
        }
      });
