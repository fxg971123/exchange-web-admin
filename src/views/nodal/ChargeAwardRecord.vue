...
import { queryChargeAwardRecord,exportChargeAwardRecord } from "@/service/getData";
...
export default {
  data() {
    return {
      ...
      currentPageIdx: 1,
      totalNum: 0,
      columns: titles.hdcTradeAwardColumns,
      rewardsList: [],
      param: {
        endTime: null,
        pageNo: 1,
  ...
  methods: {
    ...
    exportExcel() {
      exportChargeAwardRecord(this.exportParam).then((res) => {
        util.putoutTeam("交易手续费奖励", res);
      });
    },
    refreshData() {
